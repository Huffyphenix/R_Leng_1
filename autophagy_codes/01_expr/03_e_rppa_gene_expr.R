...
gene_ai_n %>%
  ...
  dplyr::mutate(size = plyr::revalue(type,replace = c('Receptor'="bold",'Ligand'="plain"))) %>%
  dplyr::select(symbol,color,size) %>%
  unique() ->gene_color_size
gene_color_size$color %>% as.character() ->gene_color_size$color
...
gene_ai_n %>%
  ...
  dplyr::select(symbol, pathway, a, i, Activation, Inhibition) %>%
...
#  dplyr::rename(pathway = pathway.x) %>% 
  ...
  tidyr::unite(col = pathway, pathway, effect) %>%
  dplyr::select(-effect_n)-> te_plot;te_plot
############
#axis text face set
############
te_plot %>% 
  ...
  dplyr::select(symbol,color,size) %>%
  unique() ->te_plot_color_size
te_plot_color_size$color %>% as.character() ->te_plot_color_size$color
...
te_plot %>% 
  ...
  geom_text(label=ceiling(te_plot$percent)) +
  ...
  theme(
    ...
    axis.text.y = element_text(color = te_plot_color_size$color,face = te_plot_color_size$size),
    axis.title.y =
    )-> p;p
